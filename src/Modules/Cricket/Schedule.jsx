...
const Schedule = () => {
  ...
  //                  DYNAMIC SEO
  // ────────────────────────────────────────────────
  const categoryLabel = categories.find(c => c.id === activeCategory)?.label || 'Cricket';

  const pageTitle = `${categoryLabel} Cricket Schedule | Upcoming Matches & Fixtures | SportlyRadar`;

  const pageDescription = `View complete ${categoryLabel.toLowerCase()} cricket schedule - upcoming matches, dates, venues, series names and timings. International tours, T20 leagues, domestic cricket & women's matches on SportlyRadar.`;

  const canonicalUrl = `https://www.sportlyradar.com/cricket/schedule${activeCategory !== 'international' ? `/${activeCategory}` : ''}`;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SEO
        ...
        keywords={`${categoryLabel} cricket schedule, upcoming cricket matches, cricket fixtures, cricket series schedule, international cricket schedule, T20 leagues schedule, domestic cricket fixtures, women's cricket matches, live cricket updates`}
        canonical={canonicalUrl}
        image="https://www.sportlyradar.com/images/cricket-schedule-og.jpg"
        url={canonicalUrl}
        type="website"
      />
