...
const LeagueDetail = () => {
  ...
  if (!league || !league.standings || !league.recentMatches || !league.topScorers) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-orange-50 to-red-50 flex items-center justify-center">
        <SEO 
          title="League Not Found | SportlyRadar"
          description="The requested basketball league could not be found."
        />
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-800 mb-2">League Not Found</h2>
          <p className="text-gray-600 mb-4">The requested league data is not available.</p>
  ...
  // SEO meta data
  const seoTitle = `${league.name} | ${league.fullName} | Basketball League | SportlyRadar`;
  const seoDescription = `Complete details for ${league.name} - ${league.fullName}. Get standings, top scorers, recent matches, and league statistics.`;
  const seoKeywords = `${league.name}, ${league.fullName}, basketball league, NBA, standings, top scorers, basketball statistics`;
  const seoImage = league.logo;
