...
//                             />
//                             <span className="font-semibold text-gray-900">{scorer.name}</span>
//                           </div>
//                         </td>
//                         <td className="px-6 py-4 text-gray-600">
//                           <div className="flex items-center gap-2">
...
//                               onError={(e) => { e.target.src = 'https://via.placeholder.com/20?text=Flag'; }}
//                             />
//                             <span>{scorer.country}</span>
//                           </div>
//                         </div>
...
const TopScorers = () => {
  ...
  // Dynamic SEO based on current filter
  const getPageTitle = () => {
    if (filter === 'all') return 'Top Goal Scorers - International Hockey';
    return `Top Goal Scorers from ${filter} - International Hockey`;
  };

  const getPageDescription = () => {
    if (filter === 'all') {
      return 'Discover the leading goal scorers in international field hockey across all countries. View rankings, goals, matches played, assists, and penalty corner goals.';
    }
    return `Top goal scorers from ${filter} in international hockey. See complete stats including goals, matches, assists, and PC goals.`;
  };

  return (
    <>
      <SEO
        title={`${getPageTitle()} | SportlyRadar`}
        description={getPageDescription()}
        keywords="top goal scorers hockey, international hockey scorers, field hockey top scorers, FIH top players, hockey goals ranking, hockey statistics, player stats hockey"
        canonical={`https://www.sportlyradar.com/hockey/top-scorers${filter !== 'all' ? `?country=${filter}` : ''}`}
        url={`https://www.sportlyradar.com/hockey/top-scorers${filter !== 'all' ? `?country=${filter}` : ''}`}
        image="https://via.placeholder.com/1200x630?text=Top+Hockey+Goal+Scorers"
      />
