...
export const Navbar = () => {
  ...
  // Structured data for breadcrumbs navigation
    ...
    "itemListElement": [
      {
        ...
        "position": 1,
        "name": "Home",
        "item": "https://sportlyradar.com"
      },
      ...sportsWithDropdown.flatMap((sport, index) => 
        sport.items.map((item, itemIndex) => ({
          "@type": "ListItem",
          "position": index * 10 + itemIndex + 2,
          "name": item.title,
          "item": `https://sportlyradar.com${item.href}`
        }))
      )
    ]
  };
  ...
  return (
    <>
      ...
      <nav 
        className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white/95 backdrop-blur shadow-sm"
        role="navigation"
          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="hidden lg:flex gap-2 items-center">
              {sportsWithDropdown.map((sport) => (
                ...
                >
                  <button 
                    className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-gray-700 hover:text-blue-600 hover:bg-blue-50 rounded-lg"
                    aria-expanded={activeDesktopDropdown === sport.name}
                    aria-haspopup="true"
                    aria-label={`${sport.name} menu`}
                  >
                    {sport.name}
                    <ChevronDown
                      className={`h-4 w-4 transition-transform ${
                  ...
                  <div
                    ...
                    role="menu"
                    aria-hidden={activeDesktopDropdown !== sport.name}
                  >
                    <div className="bg-white rounded-xl shadow-xl p-4">
                      <ul className="grid gap-1 grid-cols-1">
                        {sport.items.map((item) => (
            ...
            {sportsWithDropdown.map((sport) => (
              <div key={sport.name}>
                <button
                  onClick={() => toggleDropdown(sport.name)}
                  className="flex justify-between w-full py-3 font-semibold"
                  aria-expanded={openDropdown === sport.name}
                  aria-haspopup="true"
                >
                  {sport.name}
                  <ChevronDown
