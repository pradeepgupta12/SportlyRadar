...
const TennisScorecard = () => {
  ...
  const seoTitle = `${getPlayerShortName(matchData.players[0])} vs ${getPlayerShortName(matchData.players[1])} - Tennis Scorecard | ${matchData.tournament.name} | SportlyRadar`;
  const seoDescription = `Live tennis match between ${getPlayerName(matchData.players[0])} and ${getPlayerName(matchData.players[1])}. Get real-time set scores, match statistics, and live commentary from ${matchData.tournament.name}.`;
  const seoKeywords = `${getPlayerName(matchData.players[0])}, ${getPlayerName(matchData.players[1])}, tennis scorecard, ${matchData.tournament.name}, live tennis scores, set scores, match statistics, tennis match`;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <SEO 
        ...
        image="https://sportlyradar.com/tennis-scorecard-og-image.jpg"
        url={window.location.href}
        type="article"
        publishedTime={new Date(matchData.schedule.date).toISOString()}
      />
