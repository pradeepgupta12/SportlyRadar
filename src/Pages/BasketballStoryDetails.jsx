...
const BasketballStoryDetails = () => {
  ...
  const seoTitle = `${story.title} | Basketball News | SportlyRadar`;
  const seoDescription = story.description || `Read the latest basketball story: ${story.title}. Get insights, analysis and updates from the world of basketball.`;
  const seoKeywords = `basketball, NBA, ${story.title}, basketball news, sports story, ${story.title.split(' ').slice(0, 5).join(', ')}`;
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <SEO 
        ...
        image={story.image || "https://sportlyradar.com/basketball-story-og-image.jpg"}
        url={window.location.href}
        type="article"
        publishedTime={new Date(story.date).toISOString()}
        author="SportlyRadar Basketball Team"
      />
